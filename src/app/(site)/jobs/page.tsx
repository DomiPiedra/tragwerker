  ...
  return (
    <>
      <SitePageHeadingSection
        ...
        intro="Arbeiten Sie an anspruchsvollen Projekten im konstruktiven Ingenieurbau."
      />
            ...
            {jobs.length === 0 ? (
              ...
              jobs.map((job) => (
                <Link
                  key={job.id}
                  href={`/jobs/${job.slug}`}
                  className="flex flex-col gap-2 py-8 transition-opacity hover:opacity-70 md:flex-row md:items-center md:justify-between"
                >
                  <div>
                  ...
                  <span className="text-xs uppercase tracking-[0.2em]">Details →</span>
                </Link>
              ))
            )}
          </div>
      ...
      <SiteContactCta
        ...
        text="Keine passende Stelle? Wir freuen uns über Initiativbewerbungen."
