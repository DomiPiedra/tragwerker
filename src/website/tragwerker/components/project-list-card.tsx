...
export function FeaturedProjectCard({
  ...
  return (
    <Link href={`/projekte/${slug}`} className="group block">
      <div className="site-media-frame relative aspect-[4/3] overflow-hidden bg-[var(--site-line)] md:aspect-[8/3] lg:aspect-[4/1]">
        <CmsImage
        ...
        />
        {badge ? (
          <span className="absolute left-1/2 top-4 -translate-x-1/2 bg-[var(--site-paper)] px-3 py-1 font-site-sans text-xs font-extralight tracking-[0.2em] text-[var(--site-muted)] md:top-6">
            {badge}
          </span>
        ) : null}
      </div>
      <div className="mt-4 font-site-sans text-sm font-extralight leading-relaxed md:text-base">
...
export function ProjectGridCard({
  ...
  return (
    <Link href={`/projekte/${slug}`} className="group block">
      <div className="site-media-frame relative aspect-[4/3] overflow-hidden bg-[var(--site-line)]">
        <CmsImage
          src={heroImageUrl ?? SITE_IMAGES.projectFallback}
          alt={name}
          fill
        ...
        />
        {badge ? (
          <span className="absolute right-4 top-4 bg-[var(--site-paper)] px-2 py-1 font-site-sans text-xs font-extralight tracking-[0.2em] text-[var(--site-muted)]">
            {badge}
          </span>
        ) : null}
      </div>
