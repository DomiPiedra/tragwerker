import Link from "next/link";

import { cn } from "@/lib/utils";
import { SiteReveal } from "@/website/tragwerker/components/site-reveal";

type SiteHomeContactProps = {
  headline: string;
  text: string;
  mailto: string;
};

/** Soft dual CTA — closer to Snøhetta “Get in touch” than a solid black band. */
export function SiteHomeContact({ headline, text, mailto }: SiteHomeContactProps) {
  return (
    <SiteReveal>
      <section className="site-container border-t border-[var(--site-line)] py-20 md:py-28 lg:py-32">
        <div className="grid gap-16 md:grid-cols-2 md:gap-0">
          <article className="max-w-md border-t border-[var(--site-line)] pt-8">
            <h2 className="font-site-sans text-xl font-normal tracking-[-0.02em] text-[var(--site-ink)] md:text-2xl">
              {headline}
            </h2>
            <p className="mt-6 max-w-[65ch] font-site-sans text-base font-extralight leading-relaxed text-[var(--site-muted)]">
              {text}
            </p>
            <a
              href={`mailto:${mailto}`}
              className="site-link mt-8 inline-flex min-h-11 items-center font-site-sans text-base font-extralight tracking-[-0.01em] md:mt-10"
            >
              Kontakt aufnehmen →
            </a>
          </article>

          <article
            className={cn(
              "max-w-md border-t border-[var(--site-line)] pt-8",
              "md:border-l md:pl-12 md:ml-12 lg:pl-16 lg:ml-16"
            )}
          >
            <h2 className="font-site-sans text-xl font-normal tracking-[-0.02em] text-[var(--site-ink)] md:text-2xl">
              Offene Stellen
            </h2>
            <p className="mt-6 max-w-[65ch] font-site-sans text-base font-extralight leading-relaxed text-[var(--site-muted)]">
              Werden Sie Teil unseres Teams — an anspruchsvollen Projekten im konstruktiven Ingenieurbau.
            </p>
            <Link
              href="/jobs"
              className="site-link mt-8 inline-flex min-h-11 items-center font-site-sans text-base font-extralight tracking-[-0.01em] md:mt-10"
            >
              Aktuelle Jobs →
            </Link>
          </article>
        </div>
      </section>
    </SiteReveal>
  );
}
